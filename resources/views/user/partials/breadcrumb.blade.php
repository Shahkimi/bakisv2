{{-- Expects: string $current. Optional: array<int, array{label: string, url?: string|null}> $parents --}}
<nav aria-label="Breadcrumb" class="text-sm text-gray-500 dark:text-gray-400">
    <ol class="flex flex-wrap items-center gap-2">
        ...
        @foreach ($parents ?? [] as $segment)
            <li aria-hidden="true" class="text-gray-300 dark:text-gray-600">/</li>
            <li>
                @if (! empty($segment['url']))
                    <a href="{{ $segment['url'] }}" class="font-medium text-gray-700 dark:text-gray-200 hover:text-emerald-600 dark:hover:text-emerald-300 transition">{{ $segment['label'] }}</a>
                @else
                    <span class="font-medium text-gray-700 dark:text-gray-200">{{ $segment['label'] }}</span>
                @endif
            </li>
        @endforeach
        <li aria-hidden="true" class="text-gray-300 dark:text-gray-600">/</li>
