...
<div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
    <x-user.page-header
        current="{{ $member['nama'] ?? 'Ahli' }}"
        title="{{ $member['nama'] ?? 'Kutipan' }}"
        subtitle="Rekod pembayaran & kutipan yuran pembaharuan."
        icon="member-detail"
        variant="emerald"
        :parents="[['label' => 'Kutipan Yuran', 'url' => route('user.kutipan.index')]]"
    />
    ...
    (function () {
        ...
        async function doCollect(ev) {
            ...
            try {
                ...
                const emailNotice = typeof data.email_notice === 'string' ? data.email_notice : '';
                await Swal.fire({
                    icon: 'success',
                    title: 'Berjaya',
                    html: '<div class="text-sm">' +
                        ...
                        (emailNotice ? '<p class="mt-3 pt-3 border-t border-gray-200 dark:border-gray-600 text-gray-700 dark:text-gray-300">' + escapeHtml(emailNotice) + '</p>' : '') +
                        '</div>',
                    timer: 3500,
