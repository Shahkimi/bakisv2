...
$(document).ready(function () {
    ...
    function renderMemberCell(row) {
        ...
        let editLink = member.id
            ? `<a href="/user/members/${member.id}/edit" class="mt-1 inline-flex items-center gap-1 text-xs text-indigo-500 dark:text-indigo-400 hover:text-indigo-700 hover:underline">
    ...
    function renderStatusWithLihat(row) {
        ...
        const imgUrl = '/user/pembayaran/' + row.id + '/bukti';
    ...
    function renderActionsCell(row) {
        ...
        if (row.status === 'pending') {
            const approveUrl = '/user/pembayaran/' + row.id + '/approve';
            const rejectUrl  = '/user/pembayaran/' + row.id + '/reject';
    ...
    var table = $('#pembayaran-table').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: '{{ route("user.pembayaran.data") }}',
            type: 'GET',
    ...
    function updatePendingCount() {
        ...
        const url = '{{ route("user.pembayaran.pending-count") }}?jabatan_filter=' + encodeURIComponent(jabatanValue);
        fetch(url)
            .then(res => res.json())
            .then(data => {
