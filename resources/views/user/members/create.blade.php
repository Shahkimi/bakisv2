...
<div class="h-[calc(100vh-4rem)] max-h-[calc(100vh-4rem)] flex flex-col max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 overflow-hidden bg-gray-50 dark:bg-gray-900" x-data="{ currentStep: 1, totalSteps: 4, approvePayment: {{ old('approve_immediately') ? 'true' : 'false' }} }">
    ...
    <form action="{{ route('user.members.store') }}" method="POST" enctype="multipart/form-data" id="memberForm" class="flex-1 min-h-0 flex flex-col items-center">
        @csrf

        <div class="w-full max-w-4xl mx-auto flex-1 min-h-0 flex flex-col relative pb-4">
        @include('user.members.partials.form1')
        @include('user.members.partials.form2')
        @include('user.members.partials.form3')
        @include('user.members.partials.form4')
        ...
        <div class="flex-shrink-0 mt-6 pt-6 border-t border-gray-200 dark:border-gray-700 flex flex-wrap items-center justify-between gap-3 w-full">
            ...
            <div class="flex gap-2">
                <a href="{{ route('user.members.index') }}" class="inline-flex items-center py-2.5 px-5 text-sm font-medium rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-600 focus:ring-1 focus:ring-gray-400 transition">
                    Batal
                </a>
                <button type="button"
